...
// ============================================
        ...
        function handleFileImport(event) {
            ...
            if (!confirm('?? ATTENTION : L\'import va remplacer toutes vos donn�es actuelles.\n\nVoulez-vous cr�er une sauvegarde avant de continuer ?')) {
        ...
        function openAddChapterModal(actId) {
            // Si pas d'actId fourni, utiliser le premier acte ou on en cr�era un
            if (actId) {
                activeActId = actId;
            } else if (project.acts.length > 0) {
                activeActId = project.acts[0].id;
            } else {
                activeActId = null; // Sera cr�� dans addChapter
            }

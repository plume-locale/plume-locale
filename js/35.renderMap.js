...
// ============================================
        ...
        function renderMapView() {
            ...
            if (project.mapImage) {
                ...
                mapContent = `
                    <div style="padding: 4rem; text-align: center; background: var(--bg-secondary); border-radius: 8px; border: 2px dashed var(--border-color);">
                        ...
                        <div style="font-size: 1.2rem; font-weight: 600; margin-bottom: 0.5rem;">Aucune carte charg�e</div>
            ...
            editorView.innerHTML = `
                <div style="height: 100%; overflow-y: auto; padding: 2rem 3rem;">
                    <h2 style="margin-bottom: 2rem; color: var(--accent-gold);">??? Carte G�ographique</h2>
                    ...
                    <div style="margin-bottom: 2rem; display: flex; gap: 0.5rem; flex-wrap: wrap;">
                        ...
                        <button class="btn btn-small" onclick="exportMapData()">?? Exporter donn�es</button>
                    </div>
                    ...
                    ${project.mapLocations.length > 0 ? `
                        <div style="margin-top: 2rem; background: var(--bg-secondary); padding: 1.5rem; border-radius: 8px;">
                            <h3 style="margin-bottom: 1rem; color: var(--text-primary);">?? Lieux marqu�s (${project.mapLocations.length})</h3>
                    ...
                    <div style="margin-top: 2rem; padding: 1.5rem; background: var(--bg-secondary); border-radius: 8px; border-left: 4px solid var(--accent-gold);">
                        <p style="font-size: 0.95rem; color: var(--text-secondary); line-height: 1.6;">
                            ...
                            1. Chargez une image de carte (dessin�e � la main, g�n�r�e par IA, etc.)<br>
        ...
        function handleMapClick(event) {
            ...
            if (name) {
                ...
                saveProject();
                renderMapView();
                showNotification(`?? Lieu "${name}" ajout�`);
            }
        }
        ...
        function editMapLocation(index) {
            ...
            if (newName) {
                ...
                saveProject();
                renderMapView();
                showNotification(`?? Lieu modifi�: ${newName}`);
            }
        }
        ...
        function deleteMapLocation(index) {
            ...
            if (confirm(`Supprimer le lieu "${loc.name}" ?`)) {
                ...
                saveProject();
                renderMapView();
                showNotification(`??? Lieu supprim�: ${loc.name}`);
            }
        }
        ...
        function clearMap() {
            if (confirm('Effacer la carte et tous les lieux ?')) {
                ...
                saveProject();
                renderMapView();
                showNotification('??? Carte effac�e');
            }
        }
        
        function exportMapData() {
            const data = {
                image: project.mapImage ? 'Image pr�sente' : 'Pas d\'image',
            ...
            a.click();
            showNotification('?? Donn�es de la carte export�es');
        }

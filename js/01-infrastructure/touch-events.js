/**
 * Touch Events Service
 * Gestion des événements tactiles pour les interactions mobiles (mindmap)
 */

const TouchEventsService = (() => {
    'use strict';

    function handleNodeTouchStart(e, mindmapState, project, currentMindmapId) {
    ...
    function handleCanvasTouchStart(e, mindmapState) {
    ...
    function handleTouchMove(e, mindmapState, project, currentMindmapId) {
        const mindmap = project.mindmaps.find(mm => mm.id === currentMindmapId);
        if (!mindmap) return;
    ...
    function handleTouchEnd(e, mindmapState, project, saveCallback) {
        if (mindmapState.isDragging) {
            const mindmap = project.mindmaps.find(mm => mm.id === currentMindmapId);
            if (mindmap && saveCallback) {
                saveCallback();
            }
    ...
    function handleWheel(e, mindmapState) {
        e.preventDefault();
...
window.TouchEventsService = TouchEventsService;

console.log('[TouchEvents] Service initialisé');

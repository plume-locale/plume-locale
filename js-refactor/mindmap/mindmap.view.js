...
class MindmapView {
            ...
            <div class="mindmap-sidebar-header">
                ...
                <div style="display: flex; gap: 4px; flex-direction: column;">
                    <button class="btn btn-small" onclick="mindmapViewModel.createMindmap()" style="width: 100%;">
                        <i data-lucide="plus" style="width:14px;height:14px;vertical-align:middle;margin-right:4px;"></i> Nouvelle
                    </button>
                    <button class="btn btn-small btn-accent" onclick="mindmapViewModel.generateAutoMindmap()" style="width: 100%; background: var(--accent-gold); color: black;">
                        <i data-lucide="sparkles" style="width:14px;height:14px;vertical-align:middle;margin-right:4px;"></i> Générer Auto
                    </button>
                </div>
            </div>
        ...
        if (!mindmap) {
            editorView.innerHTML = `
                <div class="mindmap-empty">
                    ...
                    <button class="btn" onclick="mindmapViewModel.createMindmap()"><i data-lucide="plus" style="width:14px;height:14px;"></i> Créer une mindmap</button>
                </div>
            `;
            if (typeof lucide !== 'undefined') lucide.createIcons();
            return;
            ...
            <div class="mindmap-wrapper">
                <div class="mindmap-main">
                    <div class="mindmap-toolbar">
                        <button class="btn btn-small" onclick="mindmapViewModel.renameMindmap()" title="Renommer"><i data-lucide="pencil" style="width:14px;height:14px;"></i></button>
                        <button class="btn btn-small" onclick="mindmapViewModel.addNoteNode()" title="Ajouter une note"><i data-lucide="sticky-note" style="width:14px;height:14px;"></i></button>
                        <button class="btn btn-small" onclick="mindmapViewModel.resetView()" title="Réinitialiser la vue"><i data-lucide="target" style="width:14px;height:14px;"></i></button>
                        ${state.linkStart ? `
                            <button class="btn btn-small" onclick="mindmapViewModel.cancelLinking()" style="background: var(--accent-red); color: white;" title="Annuler la liaison">
                    ...
                    <div class="mindmap-canvas-wrapper ${state.linkStart ? 'linking-mode' : ''}" id="mindmapCanvasWrapper">
                        ...
                        <div class="mindmap-zoom-controls">
                            <button class="mindmap-zoom-btn" onclick="mindmapViewModel.zoom(-1)" title="Zoom arrière">
                                <i data-lucide="zoom-out"></i>
                            </button>
                            <span class="mindmap-zoom-level">${Math.round(state.zoom * 100)}%</span>
                            <button class="mindmap-zoom-btn" onclick="mindmapViewModel.zoom(1)" title="Zoom avant">
                                <i data-lucide="zoom-in"></i>
                            </button>
                            <button class="mindmap-zoom-btn" onclick="mindmapViewModel.resetView()" title="Réinitialiser la vue">
                ...
                <div class="mindmap-library ${state.libraryCollapsed ? 'collapsed' : ''}">
                    ...
                    <div class="mindmap-library-tabs">
                        <div class="mindmap-library-tab ${state.activeLibraryTab === 'characters' ? 'active' : ''}"
                             onclick="mindmapViewModel.setLibraryTab('characters')" title="Personnages"><i data-lucide="users" style="width:16px;height:16px;"></i></div>
                        <div class="mindmap-library-tab ${state.activeLibraryTab === 'elements' ? 'active' : ''}"
                             onclick="mindmapViewModel.setLibraryTab('elements')" title="Univers"><i data-lucide="globe" style="width:16px;height:16px;"></i></div>
                        <div class="mindmap-library-tab ${state.activeLibraryTab === 'codex' ? 'active' : ''}"
                             onclick="mindmapViewModel.setLibraryTab('codex')" title="Codex"><i data-lucide="book-open" style="width:16px;height:16px;"></i></div>
                        <div class="mindmap-library-tab ${state.activeLibraryTab === 'structure' ? 'active' : ''}"
                             onclick="mindmapViewModel.setLibraryTab('structure')" title="Structure"><i data-lucide="list-tree" style="width:16px;height:16px;"></i></div>
                    </div>
            ...
            return `
                <div class="mindmap-node ${typeClass} ${selectedNodeId === node.id ? 'selected' : ''} ${isLinkingSource ? 'linking-source' : ''}"
                    ...
                    <div class="mindmap-node-header">
                        ...
                        <span class="mindmap-node-link-btn" onclick="event.stopPropagation(); mindmapViewModel.startLinkFrom(${node.id})" title="Créer un lien"><i data-lucide="link" style="width:12px;height:12px;"></i></span>
    ...
    renderLibraryContent() {
        ...
        if (tab === 'structure') {
            ...
            return html || '<div style="padding: 1rem; text-align: center; color: var(--text-muted); font-size: 0.85rem;">Aucune structure</div>';
        }

            ...
            <div class="stats-container" style="height: 100%; overflow-y: auto; padding: 2rem 3rem;">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 2rem;">
                    ...
                    <div class="stat-box" style="background: var(--bg-secondary); padding: 2rem; border-radius: 8px; border-left: 4px solid var(--accent-gold);">
                        ...
                        <div style="font-size: 0.85rem; color: var(--text-secondary); margin-bottom: 1rem;">sur ${totalGoal.toLocaleString('fr-FR')} mots</div>
                        ...
                        <div style="margin-top: 1.5rem; display: flex; gap: 0.5rem; align-items: center;">
                            <input type="number" class="form-input" value="${totalGoal}" id="totalGoalInput" style="flex: 1; padding: 0.4rem;" placeholder="Objectif total">
                            <button class="btn btn-small" onclick="StatsRepository.updateGoal('totalGoal', document.getElementById('totalGoalInput').value)">Mettre à jour</button>
                        </div>
                    </div>
                    ...
                    <div class="stat-box" style="background: var(--bg-secondary); padding: 2rem; border-radius: 8px; border-left: 4px solid var(--accent-red);">
                        ...
                        <div style="font-size: 0.85rem; color: var(--text-secondary); margin-bottom: 1rem;">sur ${dailyGoal} mots</div>
                        <div style="background: var(--bg-primary); height: 8px; border-radius: 4px; overflow: hidden;">
                            <div style="background: var(--accent-red); height: 100%; width: ${dailyProgress}%; transition: width 0.3s;"></div>
                        </div>
                        <div style="margin-top: 1.5rem; display: flex; gap: 0.5rem; align-items: center;">
                            <input type="number" class="form-input" value="${dailyGoal}" id="dailyGoalInput" style="flex: 1; padding: 0.4rem;" placeholder="Objectif quotidien">
                            <button class="btn btn-small" onclick="StatsRepository.updateGoal('dailyGoal', document.getElementById('dailyGoalInput').value)">Mettre à jour</button>
                        </div>
                    </div>
                </div>
                ...
                <div style="margin-top: 3rem; display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 2rem;">
                    <!-- Stats par Acte -->
                    <div>
                        <h3 style="margin-bottom: 1rem; color: var(--text-primary); font-size: 1.1rem;">Répartition par acte</h3>
                        <div style="background: var(--bg-secondary); padding: 1rem; border-radius: 8px;">
                            ${actStats.map(act => `
                                <div style="display: flex; justify-content: space-between; padding: 0.75rem 0.5rem; border-bottom: 1px solid var(--border-color);">
                                    ...
                                    <span style="font-weight: 600; color: var(--accent-gold);">${act.wordCount.toLocaleString('fr-FR')} mots</span>
                                </div>
                            `).join('')}
                        </div>
                    </div>
                    ...
                    <!-- Historique -->
                    <div>
                        <h3 style="margin-bottom: 1rem; color: var(--text-primary); font-size: 1.1rem;">Historique (7 jours)</h3>
